...
#if defined(VITA)
// Vita's software-first build still links the hardware renderer sources, but
// the Vita std::thread backend cannot create this pool during static
// initialization. Keep the pool empty and keep the hardware BSP worker off;
// the software renderer does not use this path.
...
void HWDrawInfo::RenderBSP(void *node, bool drawpsprites)
{
	...
	#if defined(VITA)
	// Do not let a stale user config re-enable the unsupported Vita worker.
	multithread = false;
	#else
